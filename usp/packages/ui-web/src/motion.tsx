/* The prototype's motion vocabulary (ui/motion.tsx, v0.3 principle C-UX-15): springs, not timing curves; everything moves
   from where it is; one moment of reward, no more; all of it off when the user prefers reduced motion. */
...
/** How the current screen was reached: deeper (push), back (pop) or a tab switch. */
export type NavDir = 'push' | 'pop' | 'tab';

/* Damping ratios (Apple rule): critically damped by default; bounce only for momentum or a deliberate moment. */
export const SPRING = {
  soft: { type: 'spring', stiffness: 260, damping: 32, mass: 1 } as Transition,
  snappy: { type: 'spring', stiffness: 520, damping: 43, mass: 0.9 } as Transition,
  bouncy: { type: 'spring', stiffness: 430, damping: 21, mass: 0.9 } as Transition,
  gentle: { type: 'spring', stiffness: 150, damping: 24, mass: 1 } as Transition,
};
export const isRtl = () => document.documentElement.dir !== 'ltr';
...
/* ——— Screen transitions: push deeper, pop back, or switch tab ——— */
const PAGE: Variants = {
...
export function Page({ dir, children, className = '', ref }: { dir: NavDir; children: React.ReactNode; className?: string; ref?: React.Ref<HTMLDivElement> }) {
...
/* ——— Press: everything touchable responds ——— */
...
/* ——— Ticker: the number climbs to its value ——— */
...
/* ——— Check mark: drawn as at the end of a payment ——— */
...
/* ——— Island: a toast that drops from the top, says what happened, then shrinks away ——— */
...
export function IslandProvider({ children }: { children: React.ReactNode }) {
  const [msg, setMsg] = useState<(IslandMsg & { id: number }) | null>(null); const timer = useRef<number>(undefined); const seq = useRef(0);
...
/* ——— Sheet (page-level): springs up from the bottom on a phone, a centred dialog when wide; drag the grab to close ——— */
export function Sheet({ open, onClose, title, children, lead, wideDialog = true }: { open: boolean; onClose: () => void; title?: React.ReactNode; children: React.ReactNode; lead?: React.ReactNode; wideDialog?: boolean }) {
  const wide = useMedia('(min-width: 900px)') && wideDialog; const controls = useDragControls(); const reduce = useReducedMotion();
...
export function SwipeRow({ children, actions, disabled, onOpen, ariaLabel, rtl = isRtl() }: { children: React.ReactNode; actions: { label: string; icon: IconName; tone: string; onClick: () => void }[]; disabled?: boolean; onOpen: () => void; ariaLabel?: string; rtl?: boolean }) {
  ...
  // The finger's velocity is handed to the spring: no seam between drag and motion.
  const settle = (to: number, velocity = 0) => { animate(x, to, { ...SPRING.snappy, velocity }); openRef.current = to !== 0; };
...
/* ——— Tilt card: follows the pointer in perspective, flips on tap ——— */
...
export { motion, AnimatePresence, LayoutGroup, MotionConfig, useMotionValue, useSpring, useReducedMotion, useTransform, useMotionValueEvent, animate } from 'motion/react';
