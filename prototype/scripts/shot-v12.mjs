...
fs.mkdirSync('shots/v12', { recursive: true });
const ATT = (n) => path.resolve(`shots/v12/tmp/${n}`);
...
const shot = (n, full = true) => p.screenshot({ path: `shots/v12/v12-${n}.png`, fullPage: full });
...
/* ═══ ق.ح-02 مادة عامة غير متوفرة: كرسي مكتب — فرع الشراء المنقّح كاملاً حتى العهدة ═══ */
{
  ...
  ok('ق.ح-02-i', /^45000129\d\d$/.test(r.need.procurement.poNo) && r.need.procurement.poSource === 'erp' && audit(r, 'قرأته البوابة آلياً') && cur(r)?.role === 'receipt' && cur(r).assigneeIds[0] === 'P-STORE1' && (await notifs('P-DEPT', id)).some((n) => n.title.ar.includes(r.need.procurement.poNo)), `PO ${r.need.procurement.poNo} from the ERP (D-028) 4500012901 with expected ${in14} → receipt task at the general store`);
  await asPerson('P-DEPT', `#/requests/${id}`); const pg = await p.locator('.page').textContent(); await shot('02-request-stages');
  ok('ق.ح-02-j', pg.includes(pr.prNo) && pg.includes('FM-2026-0417') && pg.includes(r.need.procurement.poNo) && pg.includes('ملخص الشراء') && pg.includes('الشراء') && pg.includes('التوريد') && pg.includes('مراجع النظام المرجعي') && pg.includes('عروض أسعار'), `the request page shows the segments, the purchase summary with the offers table, and the references (reservation, PR, PO)`);
...
/* ═══ ق.ح-03 خدمة تقنية بوصف حر: رخص برمجيات (م7) — بند نصي بمجموعة أصناف ═══ */
{
  ...
  await receipt('P-ITS', id, '1000004567', null, { last: true, value: 45000 }); await receiptSign('P-MAJED', id); r = await req(id);
  ok('ق.ح-03-e', r.status === 'completed' && /^10000045\d\d$/.test(r.need.procurement.serviceEntryNo) && r.need.lines[0].status === 'delivered' && !(await getState()).custody.some((c) => c.requestId === id) && r.docs.some((d) => d.type === 'serviceReceipt' && /^REC-2026-\d{3}$/.test(d.number)) && !r.docs.some((d) => d.type === 'handover') && r.need.procurement.receipts[0].signatures.length === 2, `IT (service owner) and the buyer signed the service receipt record (entry sheet ${r.need.procurement.serviceEntryNo}) → completed, no custody, no handover note`);
...
/* ═══ ق.ح-04 خدمة غير تقنية: ترجمة (لا جهة فنية؛ المشتريات تعيّن مقدم الطلب مقيّماً) ═══ */
{
  ...
  await receipt('P-DEPT', id, '1000004568', null, { last: true }); await receiptSign('P-MAJED', id); r = await req(id);
  ok('ق.ح-04-e', r.status === 'completed' && /^10000045\d\d$/.test(r.need.procurement.serviceEntryNo) && r.docs.some((d) => d.type === 'serviceReceipt'), `service receipt record → entry sheet ${r.need.procurement.serviceEntryNo} → completed`);
}
...
/* ═══ ق.ح-05 فوق عتبة المناقصات: مناقصة إلزامية ولجنة بنصاب الكل + ق.ح-09-ج الإلغاء بعد أمر الشراء يحرّر الحجز ═══ */
{
  ...
  ok('ق.ح-05-f', stages.includes('إجراء المناقصات') && stages.includes('TND-2026-07') && /45000129\d\d/.test(stages) && stages.includes('لجنة المناقصات') === false || stages.includes('اعتماد الترسية'), `the requester sees the tender stage with its reference, the award and the PO`);
...
/* ═══ ق.ح-06 توفر جزئي: شاشتان (غير متوفرتين) وحامل (متوفر) في طلب واحد ═══ */
{
  ...
  ok('ق.ح-06-d', /^1000\d{4}$/.test(pr) && r.status === 'completed' && r.need.lines.every((l) => l.status === 'delivered') && r.docs.filter((d) => d.type === 'handover').length === 2 && r.docs.some((d) => d.type === 'inspection') && cc.length === 2, `after the purchase branch (PR ${pr}), receipt and a second handover both lines are delivered → completed; two handover notes, custody entries ${cc.length}`);
...
/* ═══ ق.ح-10 الطالب في الصورة: تنبيهات كل مرحلة، وتغيير الموعد المتوقع من المكتب ═══ */
{
  ...
  const stages = ['تحوّل إلى الشراء', 'تسلّم مكتب المشتريات', 'حُجز اعتماد', 'طُلبت العروض', 'قُيِّمت العروض', 'أُنشئ طلب الشراء', 'صدر أمر الشراء', 'وصل', 'سند التسليم'];
...
/* ═══ ق.ح-12 + ق.ح-19 مركز سياسة الاحتياج: إضافة وإلغاء بتاريخ (جهة، مستودع، فئة، مقر، بند كتالوج، شريحة صلاحيات، قواعد) في إصدار مؤرخ ═══ */
{
  ...
  const selects = p.locator('.sheet select.select-in'); await selects.nth(0).selectOption({ label: 'الأثاث والتجهيزات' }); await selects.nth(1).selectOption(''); await p.locator('.sheet input.switch').first().check(); await p.fill('.sheet textarea >> nth=0', 'أثاث المكاتب: تعتمده جهة الأثاث ويُصرف من مستودع المطبوعات والهدايا.');
...
/* ═══ ق.ح-14 بوابة المستوى: الشاشات هاتفاً وحاسوباً، عربية وإنجليزية، فاتحة وداكنة، مع تقليل الحركة ═══ */
{
  ...
  for (const [name, hash, who] of pages) {
    for (const [vpName, vp] of [['ph', ph], ['lap', lap]]) for (const lang of ['ar', 'en']) for (const dark of [false, true]) {
      ...
      if ((lang === 'en' && !dark) || (lang === 'ar' && dark)) await q.screenshot({ path: `shots/v12/v12-14-${name}-${vpName}-${lang}-${dark ? 'dark' : 'light'}.png`, fullPage: true });
      await c2.close();
    }
  }
...
fs.writeFileSync('shots/v12/v12-results.json', JSON.stringify({ at: new Date().toISOString(), checks, errors: errs }, null, 2));
await b.close();
