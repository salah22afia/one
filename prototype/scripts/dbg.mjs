...
{
  const { p, ctx, shot } = await open('dbg', lap, '#/home', { persona: 'employee', mutate: "s.balances['P-AHMED'].annual = 45; s.notifications = s.notifications.map((n) => ({ ...n, read: true }));" });
  const id = await submitLeave(p, { typeText: 'الإجازة السنوية', from: addDays(today, 40), to: addDays(today, 44), hero: true });
  console.log('id=', JSON.stringify(id), p.url()); await shot('after-submit', true);
  await setPersona(p, { persona: 'manager' }); await go(p, '#/inbox', 1500); await shot('inbox', true);
  console.log('rows', await p.locator('.row .cell').allTextContents());
  const f = await openTask(p, id); console.log('found', f); console.log((await p.locator('.split-detail').textContent().catch(()=>'')).slice(0, 400));
  console.log('btns', await p.locator('.split-detail .btn').evaluateAll((els) => els.map((e) => e.className + ' | ' + e.textContent + ' | ' + e.disabled)));
  await ctx.close();
}
await b.close();
