...
Home "Today" (hero, stories rail placeholder until Slice 5, "Needs you" with de-duplication, services dock, "Mine" widgets) · Services catalogue + search · My requests (progress, what next) · request page (status first, rail, seals) · Tasks (swipe/decision sheet, list+detail on desktop) · Notifications (threads) · Me (card + grid, data, documents wallet, balances, pay, family, settings) — all SAP data live. ⬜
